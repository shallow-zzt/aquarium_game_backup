        ...
        for (let y = 0; y < this._y; y++) {
            for (let x = 0; x < this._x; x++) {
                ...
                cube.span.addEventListener('mousedown', function (event) {
                    ...
                    if (event.buttons === 1 && cube.status === 0 && stat === 1) {
						createjs.Sound.play("tap");
                        cube.select_around();
                        leftKeyPress = true;
                    } else if (event.buttons === 2 || event.buttons === 1 && stat === 2) {
                        switch (cube.status) {
...
if (localGameData.mouse === undefined) {
    localGameData.mouse = true;
    saveGameDataToLocal();
}
//初始化游戏
Minesweeper.init(localGameData.startLevel);
Minesweeper.hint = localGameData.hint;
Minesweeper.mouse = localGameData.mouse;
//保存至本地
...
$('#m-switch').click(function () {
    let mouse = Minesweeper.mouse ? false : true;
    localGameData.mouse = Minesweeper.mouse = mouse;
    this.innerText = '鼠标标识: ' + (mouse ? '开' : '关');
	if(!mouse){
